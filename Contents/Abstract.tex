...
提出了太阳能梯级集热发电系统性能评估方法。本文针对新型梯级发电系统提出了与传统型式太阳能光热发电独立系统的对比方案，并建立了系统性能计算模型。经过建模仿真分析发现，梯级系统在较高的太阳辐射强度下，相比其对应的独立系统，具有更高的总体光电转换效率。
...
Mechanism models were established for the components of solar thermal power generation system by using mathematical calculation tool and system development tool. The modeling process uses an object-oriented approach to ensure \added[id=ZC]{independence and relevance of} each component\deleted[id=ZC]{ has both independence and relevance}. The system model has the advantages of convenient organization, clear structure, handy improvement. For component modeling, the Stirling engine modeling process, considering various irreversibilities and losses, \replaced[id=ZC]{developed}{established} a more accurate Stirling model with verification analysis. The results show that the accuracy of the \replaced[id=ZC]{developed}{newly established} Stirling model is \replaced[id=ZC]{better}{higher} than that of classical Stirling engine models.
...
A multistage heating system was proposed, which can effectively reduce the exergy loss of steam generating process. During the entire heat exchange process of a conventional steam generating system, \deleted[id=ZC]{there is} no phase change \added[id=ZC]{exists} in the heating fluid and \deleted[id=ZC]{there is} a phase change \added[id=ZC]{exists} in the heated fluid. \replaced[id=ZC]{Large}{There exist large heat transfer} temperature differences \added[id=ZC]{exist} between the two fluids in the heat exchangers, which \replaced[id=ZC]{enlarges}{makes large} exergy loss during the heat exchange process. In this thesis, a method of heating in stages \replaced[id=ZC]{was}{is} proposed, in which the flow rates of the heating fluid in different heat exchangers are controlled to reduce the heat transfer temperature difference and the exergy losses. It can effectively increase thermal efficiency of solar fields.
