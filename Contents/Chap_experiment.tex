...
第\ref{cha:Modeling}章介绍了利用MATLAB建模工具实现为太阳能光热发电系统中各关键部件建模的工作，并开发了太阳能光热系统设计软件。利用该软件，可以方便地构建太阳能光热发电实验台的模型。
...
商家给定的导热油的比热容的数据为：当$T = 373.15\mathrm{^\circ C}$时，$c_p = 2.4
】4\times10^3\,\mathrm{J/(kg\cdot K)}$；当$T = 473.15\mathrm{^\circ C}$时，$c_p = 2.88\times10^3\,\mathrm{J/(kg\cdot K)}$。本文假定导热油的比热容与温度成线性关系，采用线性插值法，则有$c_p = aT + b$，其中$a = 4.4\,\mathrm{J/(kg \cdot K^2)}$，$b = 798.14\,\mathrm{J/(kg\cdot K)}$。
...
\section{本章小结}
太阳能光热发电实验台的建立是太阳能梯级系统的建设工作的良好开端。本章介绍了实验台的部件和各回路。依据太阳辐射的特点，专门设计了特殊的实验工况来研究不同参数对槽式集热器和碟式集热器的集热效率的影响。明确了实验目的，设计了实验方案，安排了实验步骤，并进行了相关的实验，获得了相关的实验数据。
