...
为了更加清晰地表明更高的辐射强度对碟式集热器的影响，利用碟式集热器模型对碟式集热器进行了更高辐射强度条件下的模拟分析。模拟的参数为：入口空气温度为423.2$\,\mathrm{K}$，流量为0.03$\,\mathrm{kg/s}$，环境温度为283.2$\,\mathrm{K}$，环境风速为0.4$\,\mathrm{m/s}$。模拟结果如图\ref{fig:HigherDNI-eta-dish}所示。从图中可以发现，对于给定的集热器，存在最佳$I_r$，使得集热器获得最大集热效率。本实验条件下，碟式集热器的最佳$I_r$约为600$\,\mathrm{W/m^2}$。
