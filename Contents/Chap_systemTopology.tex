...
An ideal working fluid for Rankine cycle would have the temperature-entropy diagram given in \autoref{fig:idealTs}. The following characteristics listed by Abbin and Leuenberger~\cite{Abbin1977} describe this fluid:
\begin{itemize}
  \item The heat capacity of the liquid phase should be small. This makes the curve 22' in \autoref{fig:idealTs} almost vertical.
...
\autoref{fig:TypicalOrganicRankineSolarSystem} shows the schematic diagram of a typical organic Rankine cycle solar system. 
...
In the second type, air-water heat exchanger is applied to transfer heat between the air circuit and the water circuit. \autoref{fig:air-water_1} and \autoref{fig:air-water_2} show two different kinds of cascade solar systems using this type of heat exchanger. In \autoref{fig:air-water_1}, after providing heat for the Stirling engines, hot air flows through the air-water heat exchanger and provides heat for superheating. In \autoref{fig:air-water_2}, after providing heat for the Stirling engines, the hot air flows through the air-water heat exchanger and provides heat for preheating. 
...
\autoref{sec:sc} shows the idea of coupling solar chimney to concentrated solar thermal power generation technologies.
...
\autoref{sec:hebc} introduces two types of heat exchangers that may be applied in the solar thermal cascade system -- the air-oil heat exchanger (see in \autoref{fig:air-oil_c}) and the air-water heat exchanger (see in \autoref{fig:air-water_c}). 
...
\subsection{Heat recovery between cycles}
As it is mentioned in \autoref{sec:HRBC}, different thermodynamic cycles work at different optimum working temperatures. Since each thermodynamic cycle has endothermic and exothermic processes, a bottoming cycle may use the released heat of a topping cycle.
...
\label{sec:sst}
Considered all the conditions in \autoref{sec:std} and \autoref{sec:sts}, two system topologies are chosen for this research as shown in \autoref{fig:CascadeSystems}. 
