...
\nomenclature{$U$}{整体传热系数, W$\cdot$m$^{-2}\cdot$K$^{-1}$}
\nomenclature{$q''$}{热流量, W$\cdot$m$^{-2}$}
\begin{equation}
...
\nomenclature[G]{$\theta_{dc}$}{碟式集热器开口倾角 (0$^{\circ}$表示碟式集热器开口水平, 90$^{\circ}$表示碟式集热器开口竖直)}
\nomenclature[G]{$\gamma$}{拦截因子; 压缩比}
\nomenclature[G]{$\eta_{shading}$}{遮挡因子}
\nomenclature[G]{$\rho$}{反射因子}
\nomenclature{$A_{se,1}$}{斯特林机在空气侧的传热面积，$\mathrm{m^2}$}
\nomenclature{$A_{se,2}$}{斯特林机在水侧的传热面积，$\mathrm{m^2}$}
%\nomenclature{$k$}{Specific heat ratio}
%\nomenclature[G]{$\gamma_{se}$}{Compression ratio of Stirling engine}
\nomenclature{$n_g$}{单个斯特林机中的工作气体的量，mol}
\nomenclature{$n_{se}$}{斯特林机组中斯特林机的数量}
